...
public abstract class ImageWorker {
    ...
    public void loadImage(Object data, ImageView imageView) {
        ...
        if (bitmap != null) {
        	...
        	/**-----------------------------------------------------------------------------
        	 * 这里的关系是。AsyncDrawable 其实初始化显示的图片是默认的mLoadingBitmap这个bitmap。task
        	 * 在执行过程中会强制转换drable to AsyncDrawable，然后获取AsyncDrawable关联的task（下面通
        	 * 过构造函数传入的task），imageView不管task，可以imageview和task没有直接关系，但是image的
        	 * drawable和task有关系。task在执行完毕后来找imageview
        	 * -----------------------------------------------------------------------------
        	 */
     ...
     * @param cacheParams
     */
    public void addImageCache(MyApplication fragmentManager,
