...
public class MyPager extends ViewGroup {
	...
	private boolean snapToRight;
	private boolean snapToLeft;
...
//		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		Log.d("andy", "#####################onInterceptTouchEvent###################################");
		if(true){
	...
	@Override
		public boolean onTouchEvent(MotionEvent ev) {
			...
			//---------------------
			//-------------------------------------------------------------------------
			Log.d("andy", "onTouchEvent::getScrollX()="+getScrollX());
			Log.d("andy", "++++++++++++++++++++++++++"+mScroller.isFinished());
			switch (action) {
			case MotionEvent.ACTION_DOWN:
			...
			case MotionEvent.ACTION_MOVE:
				...
				Log.d("andy", "onTouchEvent::getLeft()="+getLeft());
				final int[] location = new int[2]; 
				getLocationOnScreen(location);
				Log.d("andy", "onTouchEvent::location[0]="+location[0]);
				if (mTouchState == TOUCH_STATE_REST) {
					checkStartScroll(x, y);
...
//					scrollBy(deltaX, 0);
			...
			case MotionEvent.ACTION_UP:
				...
				if (mTouchState == TOUCH_STATE_SCROLLING) {
					...
					if (velocityX > SNAP_VELOCITY ) {
						Log.d("andy", "RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR");
						if(snapToRight){
							return true;
						}else{
	...
	//					mScroller.startScroll(0, 0, getWidth(), 0, 400);
					...
					} else if (velocityX < -SNAP_VELOCITY&& !snapToLeft ) {
						if(snapToLeft){
							return true;
						}else{
							
		//					snapToPage2(mCurrentPage + 1);
								snapToDestination();
								snapToLeft = true;
								snapToRight = false;
								Log.d("andy", "llllllllllllllllllllllllllllllllllllllllllllllllllllll");
						}
					}else{
						snapToDestination();
						snapToRight = false;
...
//		Log.d("andy1", "111111111111111111111111111111111111");
//		float moveOnx = x - mLastMotionX;
//		//如果已经向右边滑动了 则不需要再滑动了
//		boolean moveToRight = moveOnx>0?true:false;
//		if(snapToRight && moveToRight)
//			return;
//		if(snapToLeft && !moveToRight)
//			return;
//		Log.d("andy1", "2222222222222222222222222222222");
	...
	void snapToPage1(int whichPage) {
		enableChildrenCache();
		...
		getLeft();
		width = width-Math.abs(getScrollX())-100;
		Log.d("andy", "width=-"+width);
		mScroller.startScroll(getScrollX(), 0, -width, 0, 800);
		invalidate();
	}
	void snapToPage2(int whichPage) {
		enableChildrenCache();
		final int newX = getScrollXForPage(whichPage);
		final int delta = newX - getScrollX();
		Log.d("andy", "getScrollX()="+getScrollX());
		mScroller.startScroll(getScrollX(), 0, 450, 0, Math.abs(delta) * 2);
		invalidate();
	}
